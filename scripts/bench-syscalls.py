...
def docker_run(command: str, *, capture: bool = False) -> subprocess.CompletedProcess[str]:
    repo = subprocess.check_output(["pwd"], text=True).strip()
    return run(
        [
            "docker",
            "run",
...
def print_report(results: dict[str, list[tuple[str, int, int]]]) -> None:
    ...
    for benchmark, syscalls in results.items():
        ...
        values = [(syscall, str(calls), str(errors)) for syscall, calls, errors in rows]
        widths = [
...
def main() -> int:
    ...
    results = {name: trace_benchmark(executable, name) for name in benchmark_names(executable)}
