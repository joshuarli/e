...
# Build the release-shaped application used by the PTY profile driver. Docker's
# profile modes keep host build scripts and the driver out of instrumentation.
pgo-instrument:
...
# Run only the ignored PTY workload. Docker's driver mode supplies native CRT
# search paths without applying profile instrumentation to the test harness.
pgo-profile: pgo-instrument
	...
	$(call cargo,driver) test --test e2e -- \
...
pgo-merge:
	...
	@$(LLVM_PROFDATA) show --counts --all-functions "$(PGO_MERGED)" | sed -n '1,24p'
	@if $(LLVM_PROFDATA) show --all-functions "$(PGO_MERGED)" | grep -Eiq 'rustybench|benchmark'; then \
