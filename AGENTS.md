...
7. **Wrap-aware render loop**: walk from `(scroll_line, scroll_wrap)` through logical lines:
   ...
   - For each wrapped chunk:
     - **Gutter**: line number (right-aligned, formatted via `write_line_num` into a stack `[u8; 20]` — no heap allocation) on first wrap row; blank on continuation rows. Current line: white bg black text (`\x1b[0;47;30m`). Other lines: dark grey (`\x1b[0;90m`).
     - **Content**: two paths:
...
`highlight_line_into(line, state, rules, out: &mut Vec<HlType>) -> HlState`:
- The primary non-allocating entry point. Clears `out`, resizes to `line.len()` (reusing capacity), dispatches to specialized highlighter, runs `highlight_semver` post-pass. All internal highlighters accept `hl: &mut [HlType]` and write in-place — no per-line allocations on steady state.

`highlight_line(line, state, rules) -> (Vec<HlType>, HlState)`:
- Thin allocating wrapper around `highlight_line_into`. Used by callers that need an owned Vec.
...
- **Benchmarks** (`benches/bench.rs`): criterion-based with a counting allocator (`CountingAlloc`) that tracks heap allocation count, total bytes, live bytes, and peak (high-water mark) with delta-from-baseline reporting. `measure_allocs(|| { ... })` helper resets counters, runs closure, returns `AllocStats { count, bytes, peak_delta }`. Also handles `realloc`. Global criterion config: 50 samples, 1s warmup, 2s measurement (via `fast_config()`). 7 groups: `gap_buffer` (from_vec, insert, pos_to_offset, offset_to_pos, line_text at 1k/10k/50k lines), `highlight` (Rust, JSON, rust_into non-allocating path at 1k/10k lines), `document` (insert+undo, interleaved edit/delete), `search` (forward/backward miss — full-scan regex at 1k/10k), `viewport` (ensure_cursor_visible, wrapped_rows), `alloc_audit` (prints `[alloc]` stats to stderr then criterion-times: highlight_into vs highlight_alloc, pos_to_offset, single_insert), `render` (full-frame rendering to a `Vec<u8>` sink — frame_80x24_1k_syntax, frame_120x40_10k_syntax, frame_120x40_10k_selection, frame_120x40_10k_plain; also prints frame byte size and allocation audit). Run with `cargo bench`.
- **Benchmark baselines** (`scripts/bench-baseline.sh`): criterion+critcmp baseline management. `--update` saves a new baseline and exports to `benchmarks/baseline.json` (committed). `--compare` compares last run against the exported baseline. Default run compares against the stored criterion baseline. Requires `critcmp` (`cargo install critcmp`).
- **Benchmark history** (`benchmarks/history/`): `bench-history.tsv` has per-commit timing and allocation data (7 commits, retroactively collected). `bench-history.md` is a rendered Markdown table view. Tracks both criterion timings and `[alloc]` audit stats.
